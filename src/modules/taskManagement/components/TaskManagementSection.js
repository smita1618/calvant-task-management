...
export default function TaskManagement({ riskFormData = {} }) {
  ...
  // Filter state (removed date filters)
  ...
  const statusOptions = [
    ...
    { value: STATUS.COMPLETED_PENDING, label: "Completed (Pending Approval)" },
  ...
  const filteredTasks = useMemo(() => {
    ...
    const assigneeText = (filters.assignee || "").toLowerCase();

  return tasks.filter((task) => {
    const status = task.status || "";
    const employeeId = String(task.employee || "");
    const employeeName = (task.employeeName || "").toLowerCase();
    ...
    const matchesAssignee =
      !filters.assignee ||
      employeeId === String(filters.assignee) ||
      employeeName.includes(assigneeText);
  ...
  const currentTasks = filteredTasks.slice(startIndex, startIndex + TASKS_PER_PAGE);

  const paginatedTasks = currentTasks.sort(
  ...
  const getStatusBadge = (status) => {
    const badges = {
      [STATUS.PENDING]: { bg: "#f39c12", color: "#fff", label: "Pending" },
      [STATUS.COMPLETED_PENDING]: { bg: "#f1c40f", color: "#000", label: "Pending Approval" },
      [STATUS.APPROVED]: { bg: "#27ae60", color: "#fff", label: "Approved" },
    };
    const badge = badges[status] || { bg: "#95a5a6", color: "#fff", label: "Unknown" };
    return (
      <span
        style={{
          padding: "6px 14px",
      ...
      <div style={{ 
        ...
        <div style={{ marginBottom: "35px" }}>
          ...
          <p style={{ 
            ...
            Create and manage mitigation tasks for identified risks
          </p>
        </div>
        ...
        <div style={{ 
          ...
          <div style={{ 
            ...
            <div style={{ 
              ...
              <div style={{ fontSize: "26px", fontWeight: "700", margin: "0" }}>
                {filteredTasks.length}
              </div>
              <div style={{ fontSize: "12px", opacity: 0.95, margin: "4px 0 0 0", fontWeight: "500" }}>
                Total Tasks
              </div>
            </div>
            ...
            <div style={{ 
              ...
              {Object.entries(statusCount).map(([status, count]) => count > 0 && (
                <div key={status} style={{ 
                  ...
                  <span style={{ color: "#718096" }}>
                    ...
                    {status === STATUS.COMPLETED_PENDING && "Pending Approval"}
        ...
        {isLoading ? (
          <div style={{ 
            ...
            <p style={{ margin: 0, color: "#718096", fontSize: "16px", fontWeight: "500" }}>
              Loading tasks...
            </p>
          </div>
        ) : (
              ...
              {paginatedTasks.map((task) => {
                ...
                return (
                  <div
                    key={task.taskId}
                    style={{
                      border: `2px solid ${isActive ? "#0052CC" : "#e2e8f0"}`,
                      borderRadius: "10px",
                      padding: "20px",
                      cursor: "pointer",
                      background: isActive ? "#f0f5ff" : "#fff",
                      boxShadow: isActive
                        ? "0 8px 24px rgba(0, 82, 204, 0.15)"
                        ...
                        <div style={{
                          ...
                          <div>
                            ...
                            <p style={{ margin: "0", fontWeight: "600", color: "#1a202c" }}>
                              {task.employeeName || task.employee || "Unassigned"}
                            </p>
                          </div>
                          <div>
                            <label style={{ 
            ...
            {/* {totalPages > 1 && (
              <div style={{
                ...
                <button
                  ...
                  onMouseOut={(e) => {
                    e.target.style.borderColor = "#e2e8f0";
                    e.target.style.background = currentPage === 1 ? "#f7fafc" : "#fff";
                  }}
                >
                  ⟨⟨
                </button>
                  ...
                  {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                    ...
                    return (
                      <button
                        ...
                        style={{
                          padding: "10px 12px",
                          border: currentPage === pageNum ? "2px solid #0052CC" : "1px solid #e2e8f0",
                          background: currentPage === pageNum ? "#0052CC" : "#fff",
                        ...
                        onMouseOut={(e) => {
                          e.target.style.borderColor = currentPage === pageNum ? "#0052CC" : "#e2e8f0";
                          e.target.style.background = currentPage === pageNum ? "#0052CC" : "#fff";
                        }}
                      >
                        {pageNum}
                      </button>
                ...
                <button
                  ...
                  onMouseOut={(e) => {
                    e.target.style.borderColor = "#e2e8f0";
                    e.target.style.background = currentPage === totalPages ? "#f7fafc" : "#fff";
                  }}
                >
                  ⟩
                </button>
                ...
                <div style={{
                  ...
                  Page <span style={{ color: "#0052CC", fontWeight: "700" }}>{currentPage}</span> of <span style={{ color: "#0052CC", fontWeight: "700" }}>{totalPages}</span>
                </div>
              </div>
            )} */}
    ...
    {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
      <button
        ...
        style={{
          ...
          border: "1px solid #ddd",
          borderRadius: "6px",
          background: currentPage === page ? "#005FCC" : "#fff",
                ...
                <div style={{
                  ...
                  <button
                    style={{
                      ...
                      borderRadius: "8px",
                      cursor: isSaving ? "not-allowed" : "pointer",
                      background: isSaving ? "#a0aec0" : "linear-gradient(135deg, #0052CC 0%, #0052CC 100%)",
                      color: "#fff",
                      border: "none",
                      fontWeight: "700",
                      fontSize: "14px",
